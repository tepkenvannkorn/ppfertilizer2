...
function insert_fb_in_head() {
	...
	if ( ! has_post_thumbnail( $post->ID ) ) { // the post does not have featured image, use a default image
		$default_image = get_stylesheet_directory_uri() . "/assets/images/default.jpg";
...
function insert_facebook_opengraph_for_homepage() {
	...
	if ( is_front_page() ) {
		...
		echo "<meta property='og:type' content='article' />";
		echo '<meta property="og:image" content="' . get_stylesheet_directory_uri() . "/assets/images/default.jpg" . '"/>';
		echo "<meta property='og:image:type' content='image/jpeg />";
		echo "<meta property='og:image:width' content='1024' />";
		echo "<meta property='og:image:height' content='768' />";
	}
